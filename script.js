...
function spinWheel() {
    ...
    function animate() {
        ...
        if (progress < 1) {
            ...
            const prizeIndex = Math.floor((360 - finalAngle) / sliceAngle) % wheelPrizes.length;
...
function checkDate() {
    ...
    if (!input) {
        resultDiv.innerHTML = '<p style="color: var(--accent);">Por favor selecciona una fecha</p>';
        return;
    }
    ...
    if (diffDays === 0) {
        resultDiv.innerHTML = '<p style="color: #38ef7d; font-size: 1.5rem;">¡Perfecto! 💕 ¡Recuerdas la fecha exacta!</p>';
    } else if (diffDays <= question.tolerance) {
        resultDiv.innerHTML = '<p style="color: #FFD700;">¡Muy cerca! ❤️ Lo importante es el recuerdo</p>';
    } else {
...
function checkSecret() {
    const input = document.getElementById('secretInput').value.toLowerCase().trim();
    ...
    if (input === 'tamo' || input === 't amo' || input === 'te amo') {
        ...
        result.innerHTML = '<p style="color: var(--accent); margin-top: 1rem;">Intenta de nuevo... 💭</p>';
    }
}
